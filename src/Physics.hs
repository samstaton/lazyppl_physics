...
model = do
  bumpers <- mapM (\_ -> do
                      x <- (sample uniform) -- used library from OPLSS/MHMonadLog.hs
                      y <- (sample uniform)
                      theta <- (sample uniform)
                      return (x*10 - 5,y*10 -5,2*pi*theta)) [1..2]
  let (x,y) = unsafePerformIO $
              do { pos <- try bumpers ; return $ last pos }
  score $ normalPdf 5 0.2 x -- used library in OPLSS/DistrLog.hs
  return bumpers
